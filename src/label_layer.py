...
from PyQt5.QtGui import QColor, QPixmap, QPainter, QPen, QImage
import numpy as np
...
class LabelLayer(QGraphicsRectItem):
    ...
    def add_mask(self, rgba: np.ndarray):
        if rgba.ndim != 3 or rgba.shape[2] != 4:
            raise ValueError("add_mask expects H×W×4 RGBA array")
        h, w, _ = rgba.shape

        if self._pixmap.isNull() or self._pixmap.size() != QPixmap(w, h).size():
            self._pixmap = QPixmap(w, h)
            self._pixmap.fill(Qt.transparent)

        qimg = QImage(rgba.data, w, h, QImage.Format_RGBA8888)
        mask_pix = QPixmap.fromImage(qimg)
    ...
    def export_pixmap(self, path: Path):
        self._pixmap.save(str(path))
